...
function renderTeoriaDelDescontrol(data) {
  ...
  container.innerHTML = `
    <div style="display: flex; flex-direction: column; align-items: center; gap: 10px; padding: 10px 0px;">
      <h3>Estado del sistema</h3>
      <div style="display: flex; flex-direction: column; gap: 5px; align-items: start; width: 100%; max-width: 400px; background: #f9f9f9; border: 1px solid #ddd; padding: 10px; border-radius: 5px;">
        <p><strong>Segundos de detección:</strong> ${data.detection_seconds}</p>
        <p><strong>LED Amarillo:</strong> ${data.led_yellow_state ? "Encendido" : "Apagado"}</p>
        <p><strong>LED Rojo:</strong> ${data.led_red_state ? "Encendido" : "Apagado"}</p>
        <p><strong>Sensor X:</strong> ${data.sensor_x_state ? "Inclinado" : "Estable"}</p>
        <p><strong>Sensor Y:</strong> ${data.sensor_y_state ? "Inclinado" : "Estable"}</p>
      </div>
    </div>`;
}
